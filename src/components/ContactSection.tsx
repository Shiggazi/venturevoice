...
export default function ContactSection({ contactEmail }: { contactEmail: string }) {
  ...
  const inputCls =
    "w-full rounded-lg border border-line-dark bg-ink-2 px-4 py-3 text-[15px] text-white placeholder:text-slate-2/60 outline-none transition-colors focus:border-cobalt";

  return (
    <section id="contact" className="canvas-grid">
      <div className="mx-auto grid max-w-6xl gap-12 px-5 py-20 md:grid-cols-2 md:py-24">
        <div>
          <p className="eyebrow text-amber">Free automation audit</p>
            ...
            {[
              ...
              <li key={x} className="flex gap-2.5">
                <svg className="mt-1 shrink-0" width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden>
                  <path d="M2 7.5l3 3 7-7" stroke="#ffb224" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                {x}
              </li>
            ))}
